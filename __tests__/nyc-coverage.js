...
const SHOULD_TEST_MODEL = true;
const SHOULD_TEST_SERVICE = false;
const SHOULD_TEST_CONTROLLER = false;

const exec = util.promisify(callbackExec);

const NPX_NYC_COMMAND = `npx nyc --reporter json-summary mocha tests/**/*.js --exit`;
...
function filterPerFolder(arr, regex) {
  return arr.filter(
    ([fileName]) => !connectionRegex.test(fileName) && regex.test(fileName),
  );
}
...
const executeTests = async () => {
  try {
    const { stdout } = await exec(NPX_NYC_COMMAND);
  } catch (error) {
...
function verifyCoverage(layer, coveragePercentage, minFunctionsQty) {
  expect(percentage(layer.total, layer.covered)).toBeGreaterThanOrEqual(
    coveragePercentage,
  );
  expect(
    percentage(layer.totalFunctions, layer.totalCoveredFunctions),
  ).toBeGreaterThanOrEqual(coveragePercentage);
  expect(layer.totalFunctions).toBeGreaterThanOrEqual(minFunctionsQty);
}

function assertCoveragePercentage(
  coverageResultsArr,
  coveragePercentage,
  minFunctionsQty,
) {
  if (SHOULD_TEST_MODEL) {
    const modelLayer = summaryCoveragePerFolder(
      filterPerFolder(coverageResultsArr, modelRegex),
    );
    verifyCoverage(modelLayer, coveragePercentage, minFunctionsQty);
  }

  if (SHOULD_TEST_SERVICE) {
    const serviceLayer = summaryCoveragePerFolder(
      filterPerFolder(coverageResultsArr, serviceRegex),
    );
    verifyCoverage(serviceLayer, coveragePercentage, minFunctionsQty);
  }

  if (SHOULD_TEST_CONTROLLER) {
    const controllerLayer = summaryCoveragePerFolder(
      filterPerFolder(coverageResultsArr, controllerRegex),
    );
